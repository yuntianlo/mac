...
// 同步读取 randconf 匿名区段里的某个键 (uci.load 完成后调用, 无 Promise)
function rc(key) {
	var s = uci.sections('randconf', 'randconf');
...
return view.extend({
	render: function() {
		...
		// 先加载相关配置, 之后 uci.sections/uci.get 均为同步取值
		return uci.load('randconf').then(function() {
			...
			o.cfgvalue = function() {
				var secs = uci.sections('system', 'system');
				return (secs && secs.length) ? (secs[0].hostname || '-') : '-';
			};
			o = s.option(form.DummyValue, '_wmac', _('WAN MAC'));
			o.cfgvalue = function() {
				var secs = uci.sections('network', 'device');
				for (var i = 0; secs && i < secs.length; i++) {
					if (secs[i].name === 'wan' && secs[i].macaddr) return secs[i].macaddr;
				}
				return '-';
			};
			...
			o.cfgvalue = function() {
				var secs = uci.sections('network', 'device');
				for (var i = 0; secs && i < secs.length; i++) {
					if (secs[i].name === 'br-lan' && secs[i].macaddr) return secs[i].macaddr;
				}
				return '-';
			};
